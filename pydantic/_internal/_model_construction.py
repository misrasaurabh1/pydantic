...
class ModelMetaclass(ABCMeta):
    ...
    def mro(cls) -> list[type[Any]]:
        ...
        generic_metadata: PydanticGenericMetadata | None = cls.__dict__.get(
            "__pydantic_generic_metadata__"
        )
        ...
        for base in original_mro[1:]:
            base_origin: type[BaseModel] | None = getattr(
                base, "__pydantic_generic_metadata__", {}
            ).get("origin")
            base_params: tuple[type[Any], ...] = getattr(
                base, "__pydantic_generic_metadata__", {}
            ).get("parameters", ())
    ...
    def __instancecheck__(self, instance: Any) -> bool:
        ...
        """
        return hasattr(
            instance, "__pydantic_validator__"
    ...
    * check for items which look like fields but are not (e.g. have no annotation) and warn.
    ...
    for var_name, value in list(namespace.items()):
        if var_name == "model_config" or var_name == "__pydantic_extra__":
            ...
            if var_name not in raw_annotations or not is_classvar(
