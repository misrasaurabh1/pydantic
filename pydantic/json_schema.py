...
class GenerateJsonSchema:
    ...
    def generate_inner(self, schema: CoreSchemaOrField) -> JsonSchemaValue:
        ...
        if 'ref' in schema:
            ...
            if core_mode_ref in self.core_to_defs_refs:
                definitions_ref = self.core_to_defs_refs[core_mode_ref]
                if definitions_ref in self.definitions:
                    return {'$ref': self.core_to_json_refs[core_mode_ref]}

        metadata_handler = _core_metadata.CoreMetadataHandler(schema)

        def populate_defs(core_schema: CoreSchema, json_schema: JsonSchemaValue) -> JsonSchemaValue:
            core_ref = CoreRef(core_schema['ref']) if 'ref' in core_schema else None
            if core_ref:
        ...
        def handler_func(schema_or_field: CoreSchemaOrField) -> JsonSchemaValue:
            ...
            if json_schema is None:
                if _core_utils.is_core_schema(schema_or_field) or _core_utils.is_core_schema_field(schema_or_field):
                    generate_method = self._schema_type_to_method[schema_or_field['type']]
                    json_schema = generate_method(schema_or_field)
                else:
        ...
        for js_modify_function in metadata_handler.metadata.get('pydantic_js_functions', ()):
            ...
            def new_handler_func(
                schema_or_field, current_handler=current_handler, js_modify_function=js_modify_function
            ):
    ...
    def tuple_schema(self, schema: core_schema.TupleSchema) -> JsonSchemaValue:
        """Generates a JSON schema that matches a tuple schema."""
        ...
        if 'variadic_item_index' in schema:
            ...
            if variadic_item_index > 0:
                ...
                json_schema['prefixItems'] = [self.generate_inner(item) for item in items_schema[:variadic_item_index]]
            if variadic_item_index + 1 == len(items_schema):
                json_schema['items'] = self.generate_inner(items_schema[variadic_item_index])
            else:
                json_schema['items'] = True
        else:
            prefixItems = [self.generate_inner(item) for item in items_schema]
            if prefixItems:
    ...
    def update_with_validations(
        ...
        """Update the json_schema with validations."""
