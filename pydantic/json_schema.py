...
class GenerateJsonSchema:
    ...
    def generate_inner(self, schema: CoreSchemaOrField) -> JsonSchemaValue:
        ...
        def convert_to_all_of(json_schema: JsonSchemaValue) -> JsonSchemaValue:
            if '$ref' in json_schema and len(json_schema) > 1:
                json_schema = {'allOf': [{'$ref': json_schema.pop('$ref')}], **json_schema}
            return json_schema
        ...
        for js_modify_function in metadata_handler.metadata.get('pydantic_js_functions', ()):
            current_handler = _schema_generation_shared.GenerateJsonSchemaHandler(
                self,
                lambda schema_or_field, ch=current_handler, jf=js_modify_function: self._handle_js_modify_function(
                    schema_or_field, ch, jf, populate_defs
                ),
            )
        ...
        for js_modify_function in metadata_handler.metadata.get('pydantic_js_annotation_functions', ()):
            current_handler = _schema_generation_shared.GenerateJsonSchemaHandler(
                self,
                lambda schema_or_field, ch=current_handler, jf=js_modify_function: self._handle_js_annotation_function(
                    schema_or_field, ch, jf, populate_defs, convert_to_all_of
                ),
            )
    ...
    def list_schema(self, schema: core_schema.ListSchema) -> JsonSchemaValue:
        ...
        Returns:
            The generated JSON schema.
        """
        items_schema = self.generate_inner(schema['items_schema']) if 'items_schema' in schema else {}
    ...
    def _handle_js_modify_function(
        self, schema_or_field, current_handler, js_modify_function, populate_defs
    ) -> JsonSchemaValue:
        ...
        if ref and json_schema:  # If ref exists and json_schema has elements to update the original schema with
            original_schema.update(json_schema)
        return original_schema

    def _handle_js_annotation_function(
        self, schema_or_field, current_handler, js_modify_function, populate_defs, convert_to_all_of
    ) -> JsonSchemaValue:
        json_schema = js_modify_function(schema_or_field, current_handler)
        if _core_utils.is_core_schema(schema_or_field):
            json_schema = populate_defs(schema_or_field, json_schema)
