...
class GenerateJsonSchema:
    ...
    def generate_inner(self, schema: CoreSchemaOrField) -> JsonSchemaValue:
        ...
        for js_modify_function in metadata_handler.metadata.get('pydantic_js_functions', ()):
            current_handler_fn = current_handler

            def new_handler_func(schema_or_field: CoreSchemaOrField) -> JsonSchemaValue:
                json_schema = js_modify_function(schema_or_field, current_handler_fn)
                if _core_utils.is_core_schema(schema_or_field):
                    json_schema = populate_defs(schema_or_field, json_schema)
                original_schema = current_handler_fn.resolve_ref_schema(json_schema)
                ref = json_schema.pop('$ref', None)
                if ref or json_schema:
