    ...
    def generate_inner(self, schema: CoreSchemaOrField) -> JsonSchemaValue:
        ...
        def handler_func(schema_or_field: CoreSchemaOrField) -> JsonSchemaValue:
            ...
            if _core_utils.is_core_schema(schema_or_field):
                json_schema = convert_to_all_of(populate_defs(schema_or_field, json_schema))
            return json_schema
        ...
        for js_modify_function in metadata_handler.metadata.get('pydantic_js_functions', ()):
            current_handler = _schema_generation_shared.GenerateJsonSchemaHandler(
                self, self.create_new_handler_func(js_modify_function, current_handler, populate_defs)
            )

        for js_modify_function in metadata_handler.metadata.get('pydantic_js_annotation_functions', ()):
            current_handler = _schema_generation_shared.GenerateJsonSchemaHandler(
                self,
                self.create_new_handler_func(js_modify_function, current_handler, populate_defs, convert_to_all_of),
            )

        json_schema = current_handler(schema)
        if _core_utils.is_core_schema(schema):
            json_schema = convert_to_all_of(populate_defs(schema, json_schema))
        return json_schema
