...
if sys.version_info >= (3, 10):
    ...
    def dataclass(
        ...
        *,
        ...
        sys_version_3_10 = sys.version_info >= (3, 10)
        kwargs = {"kw_only": kw_only, "slots": slots} if sys_version_3_10 else {}

        def make_pydantic_fields_compatible(cls: type[Any]) -> None:
            """Make stdlib dataclasses understand Pydantic Field kwargs."""
            mro = cls.__mro__
            for annotation_cls in mro:
                annotations = getattr(annotation_cls, "__annotations__", [])
                for field_name in annotations:
                    field_value = getattr(cls, field_name, None)
                    if isinstance(field_value, FieldInfo):
                        field_args = {"default": field_value}
                        if sys_version_3_10 and field_value.kw_only:
        ...
        def create_dataclass(cls: type[Any]) -> type[PydanticDataclass]:
            ...
            if _pydantic_dataclasses.is_builtin_dataclass(cls):
                ...
                if issubclass(cls, Generic):
                    bases += (Generic[cls.__parameters__],)
...
def is_pydantic_dataclass(class_: type[Any], /) -> TypeGuard[type[PydanticDataclass]]:
    ...
    """
    return (
        dataclasses.is_dataclass(class_) and "__pydantic_validator__" in class_.__dict__
    )
