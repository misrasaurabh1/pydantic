...
class FieldInfo(_repr.Representation):
    ...
    def __init__(self, **kwargs: Unpack[_FieldInfoInputs]) -> None:
        ...
        self.annotation, annotation_metadata = self._extract_metadata(
    ...
    def from_annotation(annotation: type[Any]) -> FieldInfo:
        ...
        if _typing_extra.is_annotated(annotation):
            ...
            field_info = FieldInfo.merge_field_infos(
                *field_info_annotations, annotation=first_arg
            )
            if field_info:
        ...
        return FieldInfo(
            annotation=annotation, frozen=final or None
    ...
    def from_annotated_attribute(annotation: type[Any], default: Any) -> FieldInfo:
        ...
        if isinstance(default, FieldInfo):
            default.annotation, annotation_metadata = FieldInfo._extract_metadata(
                annotation
            )  # pyright: ignore[reportArgumentType]
        ...
        if isinstance(default, dataclasses.Field):
            ...
            pydantic_field.annotation, annotation_metadata = (
        ...
        if _typing_extra.is_annotated(annotation):
            ...
            field_info = FieldInfo.merge_field_infos(
                *field_infos, annotation=first_arg, default=default
            )
            metadata: list[Any] = []
            for a in extra_args:
                if _typing_extra.is_deprecated_instance(a):
        ...
        return FieldInfo(
            annotation=annotation, default=default, frozen=final or None
        )  # pyright: ignore[reportArgumentType]
    ...
    @overload
    def get_default(
        self, *, call_default_factory: Literal[True], validated_data: dict[str, Any]
    ) -> Any: ...
        ...
        *,
        ...
        if self.default_factory is None:
            ...
            if validated_data is None:
                raise ValueError(
                    "'validated_data' must be provided if 'call_default_factory' is True."
                )
            if _fields.takes_validated_data_argument(self.default_factory):
                return self.default_factory(validated_data)
            else:
                fac = cast(
                    Callable[[], Any], self.default_factory
                )  # Pyright doesn't narrow correctly
                return fac()
        else:
            return None
    ...
    def apply_typevars_map(
        ...
        annotation = _typing_extra.eval_type(
            self.annotation, globalns, localns, lenient=True
        )
